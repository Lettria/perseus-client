...
class InterlinkService:
    def interlink(
        ...
        merge_different_entity_types: bool = False,
    ) -> "KnowledgeGraph":
        """
        ...
        Args:
            ...
            interlinking_key_uris: The URIs of the properties to use for deduplicating
        ...
        for kg in kbs:
            for entity in kg.entities:
                key_value = None
                for key_uri in interlinking_key_uris:
                    if key_uri in entity.properties:
                        key_value = str(entity.properties[key_uri].value)
                        break
                ...
                if key_value in entity_map:
                    ...
                    # New logic: Check if entity types are compatible for merging
                    if not merge_different_entity_types and existing_entity.types and entity.types:
                        # If both entities have types, and they are not identical, skip merge
                        if set(existing_entity.types) != set(entity.types):
                            logger.warning(
