...
@pytest.mark.asyncio
async def test_find_latest_job_async_existing_job_succeeded_with_refresh(
    client: PerseusClient,
):
    """
    Test that client._build_single_graph_async submits a new job when refresh_graph=True and a succeeded job exists.
    """
    file_id = "file_to_refresh"
    ontology_id = "ontology_to_refresh"
    existing_job_id = "existing_job_succeeded"
    new_job_id = "new_job_submitted"
    file_path = "dummy.txt"

    # Mock the file upload process
    mock_uploaded_file = MagicMock(id=file_id, status="COMPLETED")
    client.file.upload_file_async = AsyncMock(return_value=mock_uploaded_file)
    client.file.wait_for_file_upload_async = AsyncMock()

    # Mock find_latest_job_async to return a succeeded job
    mock_existing_job = MagicMock(id=existing_job_id, status=JobStatus.SUCCEEDED)
    client.job.find_latest_job_async = AsyncMock(return_value=mock_existing_job)

    # Mock submit_job_async to return a new job
    mock_new_job = MagicMock(id=new_job_id, status=JobStatus.PENDING)
    client.job.submit_job_async = AsyncMock(return_value=mock_new_job)

    # Mock run_job_async to complete successfully for the new job
    client.job.run_job_async = AsyncMock(
        return_value=MagicMock(id=new_job_id, status=JobStatus.SUCCEEDED)
    )

    # Mock download_job_output_async and ttl.parse_ttl_to_knowledge_graph
    client.job.download_job_output_async = AsyncMock()
    client.ttl.parse_ttl_to_knowledge_graph = MagicMock(return_value=KnowledgeGraph())

    with (
        patch("os.path.exists", return_value=True),
        patch("builtins.open", mock_open()),
    ):
        await client._build_single_graph_async(
            file_path=file_path, ontology_id=ontology_id, refresh_graph=True
        )

    client.job.find_latest_job_async.assert_called_once_with(
        file_id=file_id, ontology_id=ontology_id
    )
    client.job.submit_job_async.assert_called_once_with(
        file_id=file_id, ontology_id=ontology_id
    )
    client.job.run_job_async.assert_called_once_with(job_id=new_job_id)


@pytest.mark.asyncio
async def test_find_latest_job_async_existing_job_failed(client: PerseusClient):
    """
    Test that client._build_single_graph_async submits a new job when a failed job exists.
    """
    file_id = "file_with_failed_job"
    ontology_id = "ontology_with_failed_job"
    existing_job_id = "existing_job_failed"
    new_job_id = "new_job_submitted_after_failure"
    file_path = "dummy_failed.txt"
    ...
    # Mock find_latest_job_async to return a failed job
    mock_existing_job = MagicMock(id=existing_job_id, status=JobStatus.FAILED)
    client.job.find_latest_job_async = AsyncMock(return_value=mock_existing_job)
    ...
    with (
        ...
        await client._build_single_graph_async(
            file_path=file_path, ontology_id=ontology_id, refresh_graph=False
        )
