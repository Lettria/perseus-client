...
This command-line application replicates the process from the original notebook:

1.  **`index.py` - Build the Knowledge Graph**:
...
2.  **`explore.py` - Explore the Extracted Data**:
...
3.  **`compliance.py` - Verify Compliance**:
...
For those who wish to run the code:

1.  **Setup Environment**:
    - This project requires Python 3.8+.
    - Copy `template.env` to `.env` and fill in your Perseus API key.
    - To run the compliance checks, start the Neo4j service using Docker Compose:
      ```bash
      docker compose up -d
      ```
    - Make sure your `.env` file reflects the credentials set in `docker-compose.yaml` (default: `neo4j/password`).
...
3.  **Run the Workflow**:
    - **Build the Knowledge Graphs**:
      Process the two example documents to build the graphs and load them into Neo4j.
    ...
    - **Explore Extracted Entities**:
      See a summary of the entities extracted from the `.ttl` files.
    ...
    - **Generate a Compliance Report**:
      Run the automated ESRS E1 compliance checks against the data in Neo4j.
      ```bash
