...
def main():
    ...
    if not ttl_files:
        logging.warning(
            "No .ttl files found in the output directory. Please run `index.py` first."
        )
        return
    ...
    for ttl_file in ttl_files:
        print(f"📄 Processing RDF file: {ttl_file}")
        try:
            summary = explore_rdf_entities(ttl_file)

            print("Key climate reporting entities:")
            for type_name, count in summary["type_counts"].items():
                print(f"  • {type_name}: {count}")

            total_key_entities = sum(summary["type_counts"].values())
            print(
                f"({total_key_entities} key entities shown, {summary['total_entities']} total entities extracted)"
            )
