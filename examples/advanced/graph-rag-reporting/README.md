...
This command-line application outlines the full process:

1.  **`pdf_to_markdown.py` - Convert PDF to Markdown**:
...
2.  **`index.py` - Build the Knowledge Graph**:
...
3.  **`report.py` - Generate Context-Aware Reports**:
...
You can also upload your own documents and process them directly through the interface.
...
3.  **Run the Workflow**:
    ...
    - **Index the Document (Build the Knowledge Graph)**:
    ...
    - **Generate a Report**:
      ...
      python report.py "What are the main activities of L'Oréal?"
      ```
      You can also generate the Money KPIs report:
      ```bash
