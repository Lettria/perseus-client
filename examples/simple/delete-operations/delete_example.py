...
def demonstrate_file_deletion():
    ...
    try:
        ...
        if not found_files:
            logging.info(
                f"Verification successful: File {uploaded_file.id} was not found."
            )
        else:
            logging.warning(
                f"Verification failed: File {uploaded_file.id} still exists."
            )

    except APIException as e:
        logging.error(
            f"An API error occurred during the file deletion process: {e}"
        )
...
def demonstrate_ontology_deletion():
    ...
    try:
        ...
        logging.info(
            f"Ontology uploaded successfully. Ontology ID: {uploaded_ontology.id}"
        )

        # Step 2: Delete the ontology
        logging.info(f"Deleting ontology with ID: {uploaded_ontology.id}")
        perseus_client.delete_ontology(uploaded_ontology.id)
        logging.info("Delete command issued successfully.")

        # Step 3: Verify deletion
        logging.info(
            f"Verifying that ontology {uploaded_ontology.id} has been deleted..."
        )
        ...
        if not found_ontologies:
            logging.info(
                f"Verification successful: Ontology {uploaded_ontology.id} was not found."
            )
        else:
            logging.warning(
                f"Verification failed: Ontology {uploaded_ontology.id} still exists."
            )

    except APIException as e:
        logging.error(
            f"An API error occurred during the ontology deletion process: {e}"
        )
    except Exception as e:
        logging.error(f"An unexpected error occurred: {e}")
